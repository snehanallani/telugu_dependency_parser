...
class Parser:
    ...
    def parse(self, model, data_set, eval_batch_size=500, write_to_conll=False, output_path='', debug=False):
        """print(80*'-')
        print('MODEL')
        print(model)
        ...
        if debug:
            debug_file = open(output_path + '_debug.txt', 'w')
...
#            print("Transitions : ", [ix_to_transition[i] for i in transitions])
            ...
            for idx,parse_state in enumerate(minibatch):
                ...
                if debug:
                    debug_file.write(ix_to_transition[transition_idx] + '|\t' + str(parse_state.stack) + '|\t' + str(parse_state.buffer) + '|\t' + str(parse_state.relations) + '|\t' + ' '.join(parse_state.sentence) + '\n')
...
#        TODO write to output file in conll format
    ...
    def create_model(self):
        ...
        n_features = 3 if self.config.feature_template else 2
    ...
    def create_template_feature_vector(self, parse_state):
        ...
        for i in range(n_schildren):
            if len(stack) > i:
                ...
                word_vecs.append(self.look_up_embedding(sentence[lc[0]], self.word_vectors, lc[0], parse_state) if len(lc) > 0 else word_vec_null)
                word_vecs.append(self.look_up_embedding(sentence[rc[0]], self.word_vectors, rc[0], parse_state) if len(rc) > 0 else word_vec_null)

                if self.config.use_pos:
                    pos_vecs.append(look_up_keyed_embedding(pos_sentence[lc[0]], self.pos_embeddings) if len(lc) > 0 else pos_vec_null)
        ...
        buf_child = False
        if self.config.parsing_algo == 'eager' and buf_child:
